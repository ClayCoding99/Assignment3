...
const authUser = asyncWrapper(async (req, res, next) => {
  ...
  if (!token) {
    // return res.status(401).send("No Token: Please provide an access token using the headers.");
    throw new PokemonAuthError("No Token: Please provide the access token using the headers.")
  }

  // check if the token is valid
  let data = JSON.parse(base64.decode(token.split('.')[1]));
  //find the user in the database
  const user = await userModel.findOne({username: data.user.username, password: data.user.password});

  console.log("THIS IS THE DATA");
  console.log(user);
  if (user?.token_invalid) {
    // return res.status(403).send("Invalid Token: Please ensure you are signed in.");
    throw new PokemonAuthError("Invalid Token: Please ensure you are signed in.");
  }
  ...
  try {
    ...
    return res.status(403).send("Invalid Token: Please provide a valid access token.");
  }
})
